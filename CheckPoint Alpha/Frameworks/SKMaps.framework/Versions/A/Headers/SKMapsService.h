...
@class SKMapsInitSettings, SKProxySettings, SKMapInternationalizationSettings,SKTileAccessService;

/** SKMapsService class provides support for general settings and management that concerns the whole SKMaps Framework behavior.
 */
...
/** Initializes the SKMaps Framework with an API key and settings. Should be called once, usually when the application starts ( application:didFinishLaunchingWithOptions: method from the AppDelegate ). When the library is fully initialised, a kSKMapsLibraryInitialisedNotification notification will be posted.
 ...
 */
@property(nonatomic,readonly,strong) SKTileAccessService *tileAccessService;
